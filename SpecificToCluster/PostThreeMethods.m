...
%load([SavePath 'ResGenBRBert10_BRdeterm_' num2str(i) '.mat'])
    ...
    partFirstObsALL{1}(cpt,1:NPartsDet) = FtsDet(:,472);
    partFirstObsALL{2}(cpt,1:NPartsAdd) = FtsAdd(:,472);
    partFirstObsALL{3}(cpt,1:NPartsBer) = FtsBer(:,472);
    partFirstObsALL{5}(cpt,1:NPartsSigm) = FtsSigm(:,472);
    partFirstObsALL{6}(cpt,1:NPartsSigmSto) = FtsSigmSto(:,472);
    ...
    for j = 1:length(qs)    
      ...
      partFirstObsROC(1,cpt) = quantile(partFirstObsALL{1}(cpt,1:NPartsDet),qs(j));
      partFirstObsROC(2,cpt) = quantile(partFirstObsALL{2}(cpt,1:NPartsAdd),qs(j));
      partFirstObsROC(3,cpt) = quantile(partFirstObsALL{3}(cpt,1:NPartsBer),qs(j));
      partFirstObsROC(4,cpt) = FtTrue(472);
      partFirstObsROC(5,cpt) = quantile(partFirstObsALL{5}(cpt,1:NPartsSigm),qs(j));
      partFirstObsROC(6,cpt) = quantile(partFirstObsALL{6}(cpt,1:NPartsSigmSto),qs(j));
...
Res.partFirstObs = partFirstObsROC;
