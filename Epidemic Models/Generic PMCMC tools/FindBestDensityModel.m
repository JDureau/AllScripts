...
elseif size(Data,2) == 30
    ...
    for i = 1:k
        ...
        xs = min(Data(:,i)):(max(Data(:,i))-min(Data(:,i)))/100:max(Data(:,i));
        fs = zeros(size(xs));
        for IndComp = 1:temps{ind}.NComponents
            m = temps{ind}.mu(IndComp,indi) ;
            std = sqrt(temps{ind}.Sigma(indi,indi,IndComp));
            fs = fs+temps{ind}.PComponents(IndComp)*normpdf(xs,m,std);
        end
        plot(xs,fs,'g')
        hold off
    end
