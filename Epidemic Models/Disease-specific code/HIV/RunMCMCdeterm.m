...
%     xstar = mvnrnd(x,Epsil^2*tmpCov); 
    ...
    if IndModel == 1
        TempSimStar = SimulateBertallanfyDetermEpid(Data,Parameters,HIVModel);
    elseif IndModel == 2
        TempSimStar = SimulateSigmoidDetermEpid(Data,Parameters,HIVModel);
    end
    tmpLogLik = 0;
    tmpLogPrior = 0;
    tmpLogCorr = 0;
    for i = 2:length(Data.ObservedVariables)
        tmpLogLik = tmpLogLik + max(-700,not(TempSimStar.Crash)*log(normpdf(Data.Observations(Data.ObservedVariables(i),i),TempSimStar.Observations(Data.ObservedVariables(i),i),sqrt(Data.Observations(Data.ObservedVariables(i),i)*(100-Data.Observations(Data.ObservedVariables(i),i))/400))));
    end
    for i = 1:length(NamesEst)
        tmp = Parameters.(NamesEst{i}).Prior(NamesEst{i},Parameters);
        tmpLogPrior = tmpLogPrior + log(tmp);
