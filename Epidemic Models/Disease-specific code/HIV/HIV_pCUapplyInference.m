function Res = HIV_pCUapplyInference(Data,Parameters)
...
%             HIVModel.ObservationMeasurementNoise{i+1} = ((Parameters.ObsMax(i)-Parameters.ObsMin(i))*100/4)^2;%(Data.Observations(ObsVars(i),i+1)*(100-Data.Observations(ObsVars(i),i+1))/400);
        ...
        temps{7} = temp7;
        temps{8} = temp8;
        HIVModel0.ObservationJacobian = {};
        for i = 1:length(ObsVars)
            HIVModel0.ObservationJacobian{i+1} = temps{ObsVars{i}(1)};
        end
        HIVModel0.ObservationMeasurementNoise = {};
        for i = 1:length(ObsVars)
            for j = 1:1%length(ObsVars{i})
%             HIVModel.ObservationMeasurementNoise{i+1} = ((Parameters.ObsMax(i)-Parameters.ObsMin(i))*100/4)^2;%(Data.Observations(ObsVars(i),i+1)*(100-Data.Observations(ObsVars(i),i+1))/400);
                HIVModel0.ObservationMeasurementNoise{i+1}(j) = (Parameters.Obs{i}(j)*100*(100-Parameters.Obs{i}(j)*100)/Parameters.NbSamples(i));
            end
        end
       Data0 = Data;
       for i = 1:length(ObsVars)            
            Data0.ObservedVariables{i+1} = Data0.ObservedVariables{i+1}(1); 
%             Data.ObsSigmas(i+1) = ((ObsMax{i}(2)-ObsMin{i}(j))*100/4);
        end
...
if not(AlreadySomething)
    ...
    [x,fval,exitflag,output] = fminsearch(@(x) KalmanToOptimizeWithPrior(x,Data0,HIVModel0,ParametersKalman),Initialization,optimset('MaxIter',2000,'TolX',1e-8,'TolFun',1e-8,'MaxFunEvals',4000));
    Names = ParametersKalman.Names.Estimated;
    for i = 1:length(Names)
    ...
    ParametersKalman.Rho.Value = 1.05;
    ParametersKalman = UpdateParsNoTransfToTransf(ParametersKalman);
