...
%     clf
    ...
    hold on
    plot(IndTime*NbIts ,mpred(plind)+sqrt(Cov(plind,plind)),'+c')
    plot(IndTime*NbIts ,mpred(plind)-sqrt(Cov(plind,plind)),'+c')
...
%                 [xm Cov] = UT(msigm,W)
        ...
        msigm = msigm + (repmat(fmean,1,2*L+1)+ [zeros(L,1) sqrt(L+kappa)*tmp -sqrt(L+kappa)*tmp])*TStep;
...
%         [IndTime Cov(3,3)]
        ...
        plot(IndTime*NbIts + IndDiscr ,mpred(plind)+sqrt(Cov(plind,plind)),'+c')
        plot(IndTime*NbIts + IndDiscr ,mpred(plind)-sqrt(Cov(plind,plind)),'+c')
%         mpred(1)
%        
%         die
