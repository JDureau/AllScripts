...
addpath([pwd '/Epidemic Models/Disease-specific code/SIRsimf'])
...
Data.Observations = zeros(9,53);
Data.Observations(7,2:end) = data(:,1);
Data.Observations(8,2:end) = data(:,2);
...
tmp = zeros(2,53);
for i = 2:53
    if not(isnan(Data.Observations(7,i)))
        tmp(1,i) = 7;
    end
    if not(isnan(Data.Observations(8,i)))
        tmp(2,i) = 8;
    end
end
Data.ObservedVariables = tmp;
Parameters.NbVariables = 9;

clf
plot(Data.Instants,Data.Observations(7,:))


SIRsimfModel.InitializeParameters = @SIRsimf_Initialize;
SIRsimfModel.EKF_projection = @SIRsimf_EKF_projection;
SIRsimfModel.ObservationMeasurementNoise = {};
...
clf
Result = EstimationEKFGen(Data, SIRsimfModel, Parameters);
...
Parameters = KalmOpt(Parameters,Data,SIRsimfModel,2000);
