function Res = SEIR_EKFSpline_projection(Data,Model,m,Cov,NbIts,IndTime,Parameters)
    ...
    for i = 1:length(Data.Instants)
        ...
        Model.ObservationMeasurementNoise{i} = (Parameters.SigmaObs.Value*Data.Observations(5,i)*coeff)^2;
    end
...
%             die
%         end
        beta = Parameters.Betas(sum(Data.NbComputingSteps(IndTime))+IndDiscr);
...
%         end
        Q = zeros(7,7);
        if strcmp(Parameters.DiffusionType,'Add')
            Q(1:4,1:4) =eye(4)*(Parameters.SigmaRW.Value)^2;
