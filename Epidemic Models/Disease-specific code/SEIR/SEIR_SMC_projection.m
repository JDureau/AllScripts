function Res = SEIR_SMC_projection(Variables,IndIt,NbIts,Data,Parameters,Path)
    ...
    for IndDiscr = 1:NbIts
        ...
        elseif Parameters.DiffusionType =='Add'
            TempVariables(:,6) = TempVariables(:,6) + sqrt(ComputationTStep)*SigmaLambda*rands(:,IndDiscr);
        elseif Parameters.DiffusionType =='IBM'
            TempVariables(:,7) = TempVariables(:,7) + sqrt(ComputationTStep)*SigmaLambda*rands(:,IndDiscr);
            TempVariables(:,6) = TempVariables(:,6) + ComputationTStep * TempVariables(:,7);
        elseif Parameters.DiffusionType =='SVO'
            TempVariables(:,6) = TempVariables(:,6) + sqrt(ComputationTStep)*Variables(:,7).*rands(:,IndDiscr);
            TempVariables(:,7) = TempVariables(:,7) + sqrt(ComputationTStep)*SigmaLambda*rands(:,IndDiscr);
        else
