function Res = SEIR2_3diff_SMC_projection(Variables,IndIt,NbIts,Data,Parameters,Path)
    ...
    rands = randn(4,Parameters.NbParticules ,NbIts);
    ...
    for IndDiscr = 1:NbIts
        ...
        beta12 = exp(TempVariables(:,13));
        beta21 = exp(TempVariables(:,14));
        %S
        ...
        TempVariables(:,13) = TempVariables(:,13) + sqrt(ComputationTStep)*SigmaRW12*squeeze(rands(3,:,IndDiscr))';
        TempVariables(:,14) = TempVariables(:,14) + sqrt(ComputationTStep)*SigmaRW21*squeeze(rands(4,:,IndDiscr))';
