...
%         pause(0.01)
        ...
        CorrecPrior = ones(NbParticules,1);
        ...
        CorrectedWeigths = Weigths/sum(Weigths).*CorrecPrior;
        ResampledIndexes = Resample(CorrectedWeigths);
...
%         disp(mean(NoTransfThetasNotInit(:,end)))
        ...
        LogLiksTemp(IndTime) = log(sum(LogLikWeigths)/NbParticules) + NbIterations*mean(log(CorrecPrior));
