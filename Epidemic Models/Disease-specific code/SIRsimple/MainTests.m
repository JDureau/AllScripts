...
addpath([pwd '/Epidemic Models/Disease-specific code/SIRsimple'])



Data = struct();
SavePath = '/Users/dureaujoseph/Documents/Taf/These/Matlab Scripts/AllData/SimfSimple/';
load([SavePath '/Parameters.mat']);
data = load([SavePath '/TutoData.txt']);

%% generate data


Data.Observations = zeros(4,53);

p_t = Parameters.p_t;
mpred = [Parameters.Sinit.Value*p_t Parameters.Iinit.Value*p_t 0 log(4)]';
TStep = 1/100;
r0 = Parameters.r0_init.Value;
...
for i = 2:53
    ...
    Data.Observations(3,i) = Parameters.rep2.Value*mpred(3);
end
plot(Data.Observations(3,:))
Data.ObservedVariables = 3*ones(1,53);
Parameters.ComputationTStep = 0.01;
Data.Instants = (1:53)/Parameters.ComputationTStep;
...
SIRsimpleModel.InitializeParameters = @SIRsimple_Initialize;
SIRsimpleModel.EKF_projection = @SIRsimple_EKF_projection;
SIRsimpleModel.ObservationMeasurementNoise = {};

Parameters.ComputationTStep = 0.1;
Data.Instants = (1:53)/Parameters.ComputationTStep;
Data.NbComputingSteps = [0 diff(Data.Instants)];
Parameters.NbVariables = 4;
Parameters.MultCoeff.Value = 0.6;

clf
Result = EstimationEKFGen(Data, SIRsimpleModel, Parameters)


Parameters.Problem = 'SIRsimple';
Parameters.Correction = 1;
Parameters = KalmOpt(Parameters,Data,SIRsimpleModel,2000);



subplot(2,1,1)
plot(Data.Observations(7,:),'g')
hold on
plot(Result.PosteriorMeans(7,:))
hold off
subplot(2,1,2)
plot(Data.Observations(8,:),'g')
hold on
plot(Result.PosteriorMeans(8,:))
hold off
title(Result.LogLik)
